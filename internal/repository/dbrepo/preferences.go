...
func (m *postgresDBRepo) SetSystemPref(name, value string) error {
	...
	stmt := `UPDATE preferences SET preference = $1 WHERE name = $2`

	result, err := m.DB.ExecContext(ctx, stmt, value, name)
	if err != nil {
		log.Println(err)
		return err
	}
	...
	//no update, insert a new row
	if rows != 1 {
		stmt = `insert into preferences (name, preference, created_at, updated_at) values ($1, $2, $3, $4)`
		_, err = m.DB.ExecContext(ctx, stmt, name, value, time.Now(), time.Now())
	}
